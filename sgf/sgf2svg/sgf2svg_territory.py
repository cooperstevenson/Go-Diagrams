    ...
    'default': { # {{{
       ...
       #arrHead {
        ...
        'sgf_defs': '''
            ...
            <symbol id="color" viewBox="0 0 1 1">
                <rect class="color" x="0" y="0" width="1.007" height="1.007" />
            </symbol>
            <symbol id="territory" viewBox="0 0 1 1">
                <rect class="territory" x="0" y="0" width="1" height="1" />
            </symbol>
<marker id="arrHead" orient="auto" refX="1" refY="2"
                    markerWidth="5" markerHeight="5">
                <polygon points="4,2 0,4 1,2 0,0" />
...
def make_svg(board, options, theme):
    ...
    stone_tags = []
    marker_tags = []
    terr_file = options.input_file
    # Get color array data
    color_array = []
    ins = open( terr_file, "r" )
    for line in ins:
       #stripped_line = line.strip('\n')
       stripped_line = line.split(' ')
       color_array.append( stripped_line )
       # ins.close()
    ...
    #        xp, yp = ( options.board_padding + p*options.stone_size for p in (x, y) )
    #        cls='color'
    #        color_tags.append(svg_tag('use', xlink_Chref='#color', fill=color_array[x][y], stroke=color_array[x][y], _class=cls, 
    #        x=xp, y=yp, width=options.stone_size, height=options.stone_size ))
    ...
    for x in range(board.width):
        for y in range(board.height):
            ...
            # works in inkscape but wrong in browsers
            # xp, yp = ( options.board_padding + (p+0.5)*options.stone_size for p in (x, y) )
            
            #correct in browsers
            #xp, yp =  ( options.board_padding + (p+.5)*options.stone_size -abs(float(color_array[x][y])*5) for p in (x, y) )
            xp, yp =  ( options.board_padding + (p+.5)*options.stone_size -abs(float(color_array[y][x])*5) for p in (y, x) )
            cls='territory'
            # I have no idea what I'm doing--swapped the 'x' and 'y' values
            if float(color_array[x][y]) <= 0:
                territory_tags.append(svg_tag('use', xlink_Chref='#territory', fill='#ffffff', stroke='#ffffff', _class=cls, y=yp, x=xp, width=(10 * (abs( float(color_array[y][x]) ))), height=(10 * abs(float(color_array[y][x]))))) 
            else:
                territory_tags.append(svg_tag('use', xlink_Chref='#territory', fill='#000000', stroke='#000000', _class=cls, y=yp, x=xp, width=(10 * abs( float(color_array[y][x])  )), height=(10 * abs(float(color_array[y][x])))) )
...
def main():
    ...
    parser.add_option('-i', '--input-file', help='input criticality file', metavar='CRITFILE')
