...
<symbol id="markerCR" viewBox="-9.5 -9.5 19 19">
            ...
            <symbol id="territory" viewBox="0 0 1 1">
                <rect class="territory" x="0" y="0" width="1.007" height="1.007" />
            </symbol>
<marker id="arrHead" orient="auto" refX="1" refY="2"
                    markerWidth="5" markerHeight="5">
                <polygon points="4,2 0,4 1,2 0,0" />
...
def make_svg(board, options, theme):
    ...
    stone_tags = []
    marker_tags = []
    #critical area file name
    crit_file = options.input_file

    # Get color array data
    color_array = []
    ins = open( crit_file, "r" )
    for line in ins:
    ...
    for x in range(board.width):
        for y in range(board.height):
            yp, xp = ( options.board_padding + p*options.stone_size for p in (y, x) )
            cls='color'
            color_tags.append(svg_tag('use', xlink_Chref='#color', fill=color_array[y][x], stroke=color_array[y][x], _class=cls, 
            y=yp, x=xp, width=options.stone_size, height=options.stone_size ))
    ...
    #            territory_tags.append(svg_tag('use', xlink_Chref='#territory', fill='#ffffff', stroke='#ffffff', _class=cls,               
    #            x=xp, y=yp, width=(10 * (abs( float(color_array[x][y]) ))), height=(10 * abs(float(color_array[x][y])))))
    #
        #        else:
            #        territory_tags.append(svg_tag('use', xlink_Chref='#territory', fill='#000000', stroke='#000000', _class=cls,
                #    x=xp, y=yp, width=(10 * abs( float(color_array[x][y])  )), height=(10 * abs( float(color_array[x][y])))) )
